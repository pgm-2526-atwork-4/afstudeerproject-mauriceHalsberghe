...
export default function LikedRecipes() {
  ...
  useEffect(() => {
    ...
    const fetchLikedRecipes = async () => {
      ...
      try {
        ...
        const onlyLiked = data.recipes.filter((r: Recipe) => r.isLikedByCurrentUser);
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    const matchesDiet =
      filters.selectedDiet === 0 || recipe.diet?.id === filters.selectedDiet;

    const matchesCuisine =
      filters.selectedCuisine === 0 || recipe.cuisine?.id === filters.selectedCuisine;

    const matchesTitle =
      filters.search === "" ||
      recipe.title.toLowerCase().includes(filters.search.toLowerCase());

    const matchesOnlyUsers = !filters.onlyUsers || recipe.user !== null;

    const matchesOnlyInStock =
      !filters.onlyInStock || recipe.missingIngredientCount === 0;

    return matchesDiet && matchesCuisine && matchesTitle && matchesOnlyUsers && matchesOnlyInStock;
  });
    ...
    <main className={HomeStyles.home}>
      ...
      {recipes.length === 0 ? (
        ...
        <EmptyView
          ...
          text="No liked recipes match your search"
          icon="recipe"
        />
      ) : (
